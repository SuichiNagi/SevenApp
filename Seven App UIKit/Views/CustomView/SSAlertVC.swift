...
class SSAlertVC: UIViewController {
    ...
    lazy var titleLabel: SSTitleLabel = {
        let label  = SSTitleLabel(textAlignment: .center, fontSize: 20)
        label.text = alertTitle ?? "Something went wrong"
        return label
    }()
    
    lazy var actionButton: SSButton = {
        let button = SSButton(backgroundColor: .systemPink, title: "Ok")
    ...
    lazy var bodyMessageLabel: SSBodyLabel = {
